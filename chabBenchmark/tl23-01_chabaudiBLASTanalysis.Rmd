...
times_stages_table <- read_tsv(file = 'samples_lanes_rMT.list', col_names = FALSE) %>% 
  ...
  mutate(time = format(strptime(str_extract(X1, pattern = '(?<=Pc).+(?=-)'), "%I%p"), "%Hh"),
         ...
         stage_diff_count = uniquify(stage_diff_count)) %>% 
...
left_join(head(blast, n = 1000), tpm_time.df, by = c('geneid', 'sample_name'))

left_join(head(blast, n = 1000), times_stages_table, by = c('sample_name'))
```
