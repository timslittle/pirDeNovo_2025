...
names(transcripts_covered_by_denovo) <- names(blast)
lapply(transcripts_covered_by_denovo, getElement, 'possible_chimeras')
transcripts_covered_by_denovo$`21659_8#9_spadesSC`
transcripts_covered_by_denovo$`21659_8#9_evigene`
```
...
lapply(transcripts_covered_by_denovo[1:10], 
       function(blast_list) {
  blast_list$blast_df %>% 
  filter(qseqid %in% unique(blast_list$transcripts_covered_by_denovo_df$qseqid)) %>%
  group_by(qseqid) %>%
  mutate(num_match_all = n_distinct(sseqid)) %>% 
...
transcripts_covered_by_denovo$`21659_8#1_evigene`$blast_df %>% 
  group_by(qseqid) %>%
  mutate(num_match_all = n_distinct(sseqid)) %>% 
  mutate(qseqid = paste0(qseqid, '; n = ', num_match_all),
         over95 = ifelse(scovhsp >= 95, 
       ...
       function(sample){
         ...
         ggplot(data = cvrg_df) +
           geom_line(aes(y = num_transcripts, 
                         ...
                         col = assembler),
                     size = .75) +
           scale_linetype_manual(values = c('100' = 2, '90' = 1, '75' = 2), 
                                 name = '% identity') +
           theme_classic() +
...
unique(data.table(transcripts.df), by = c("sample","sseqid")) %>% 
