...
blast_forcov.df %>% 
  ...
  group_by(sample_name, assembler_name) %>% 
  slice_max(scovhsp) %>% 
  mutate( max(qcovhsp), max(pident)) %>% 
  ungroup() %>% 
  select(sample_name, assembler_name, sseqid, scovhsp, 'max(pident)', 'max(qcovhsp)' ) %>% 
  arrange(scovhsp)
```
...
```{r}
sample_table <- read.delim('samples_lanes_rMT.list', col.names = c('sample','transmission','id') ) %>% 
...
transcripts_tpm <- merge(blast_forcov.df, sample_tpm, by = c('sample_name', 'Gene.ID') )
```
