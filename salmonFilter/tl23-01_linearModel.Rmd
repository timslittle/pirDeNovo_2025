  ...
  mutate(helpful_id = paste0(experiment_id, '_', helpful_assembly_name),
         ...
         assembly_id = paste0(experiment_id,'_',
                              str_extract(assembly,
                                          pattern = '(?<=_).+(?=_)'))) %>%
  select(assembly_id, num_pir, lifecycle_stage, location, experiment_id, helpful_assembly_name, helpful_id, country)
...
missing_assemblies.df <- filter(assembly_stats.df, 
...
group_by(lm.df, assembly_id) %>% summarise(n_distinct(Complete))
```
