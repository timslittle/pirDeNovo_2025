...
mod.df <- bind_rows(mod.df, missing_assemblies.df) %>% 
  ...
  # Fill in the missing labels for the missing data we are adding.
...
# group_by(mod.df, assembly_id) %>% summarise(total_tpm = sum(TPM)) %>% arrange(desc(total_tpm)) %>% tail()
```
...
mod.df <- group_by(mod.df, 
  ...
  group_by(sample) %>% 
  mutate(prop_TPM = round(TPM/sum(TPM), digits = 4)) %>% 
  #I admit that I don't know why there are NAs here
  filter(!is.na(sample)) %>% 
  arrange(desc(prop_TPM))

# group_by(mod.df, sample) %>% summarise(n = n_distinct(new_subfam_uniq)) %>% arrange(desc(n))
```
