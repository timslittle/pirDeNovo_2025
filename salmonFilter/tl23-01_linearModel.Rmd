...
negbin_lifecycle_model <- glm.nb(TPM_med ~ new_subfam_uniq * country * lifecycle_stage + Complete, 
                                 data = bin.df)
new.df <- mutate(bin.df, busco_extra = ifelse(Complete > 50, 'High', 'Low'))

negbin_xtraxtraxtra_model <- glm.nb(TPM_med ~ new_subfam_uniq * country * lifecycle_stage + Complete * experiment_id + busco_extra, 
                                 data = new.df)
negbin_xtraxtraxtraxtra_model <- glm.nb(TPM_med ~ new_subfam_uniq * country * lifecycle_stage + Complete * experiment_id * busco_extra, 
                                 data = new.df)
...
lr_test <- lrtest(poisson_model, poisson_model_interaction, poisson_lifecycle_model,
                  ...
                  ,negbin_xtra_model,negbin_xtraxtra_model, negbin_xtraxtraxtra_model,negbin_xtraxtraxtraxtra_model)
summary(negbin_xtra_model)
print(lr_test)
...
```{r}
plot(select(ungroup(bin.df), where(is.double) ))
```
