...
## Sub-family distributions across the *number*s of _pir_\s from different *species*.
...
## Sub-family distributions across the *number*s of _pir_\s from different *assemblies*.
...
quant_subfam.df <- left_join(quant.df, 
...
lapply(unique(quant_subfam_buscoFifty.df$experiment_id), function(expt.id){
  ...
  lapply(unique(expt.df$assembly_id), function(assm.id){
    assm.df <- expt.df[expt.df$assembly_id == assm.id,] %>% filter(TPM_max >= 20)
    if(nrow(assm.df) == 0) {return(NULL)}
    tpm.df <- reshape2::dcast(assm.df, label + new_subfam_uniq + TPM_med ~ sample, value.var = 'TPM') %>% 
