...
read_HMMertxtgz <- function(hmmer_gz.path) {
  hmmer_gz.df <- readLines(hmmer_gz.path)
  header.index <- first(which(grepl(hmmer_gz.df, 
                                    pattern = 'E-value')))
  end.index <- first(which(grepl(hmmer_gz.df, 
                                 pattern = 'inclusion threshold ')))
  hmmer_gz.df <- hmmer_gz.df[c(header.index,
                               (header.index+2):(end.index - 1))]
  
  hmmer_gz.df <- hmmer_gz.df[c(header.index,
                               (header.index+2):(end.index - 1))]
  hmmer_gz.df <- sapply(hmmer_gz.df, function(line){
    #Split by whitespace and remove the first element (which is blank)
  ...
  hmmer_gz.df <- lapply(hmmer_gz.df[-1], 
                        function(line){
                          data.frame(t(unlist(c(line[1:num_cols - 1], paste(line[num_cols:length(line)], collapse = "")))))
                        }) %>% bind_rows
...
experiment.vec <- c('Bourgard21', 'Imp22', 'K19', 'Meul22', 'Meul22-PNG',
...
quant.df <- lapply(experiment.vec,
                   function(experiments){
                     
                     results.path = paste0("../../nextflow/vvx_",
                                           experiments,
                                           "_results")
                     
                     quant_files <- list.files(path = file.path(results.path, "salmon"),
                                               recursive = TRUE,
                                               pattern = 'quant.sf',
                                               full.names = TRUE) %>% 
                       str_subset(pattern = '.+T[[:digit:]].+')
                     
                     #Some of the files are from erroneous runs of the software, can grep for the 'T1' etc name to find those ran with the more 
                     # modern pipeline. 
                     #TODO: Remove the old files.
                     
                     #Used different naming parameters for some files like an eejit, need to find these specifically:
                     
                     sample.pattern <- case_when(
                       experiments == 'Bourgard21' ~ '(?<=/salmon/).+(?=(_Bourgard).+/quant.sf)',
                       grepl(experiments, pattern = 'Meul22') ~ '(?<=/salmon/)vvx_.+_T[[:digit:]]{1,2}(?=_vvx.+/quant.sf)',
                       .default = '(?<=/salmon/).+_vvx_T[[:digit:]]{1,2}(?=.+_vvx/quant.sf)'
                     )
                     
                     assembly.pattern = case_when(
                       experiments == 'Bourgard21' ~ '(?<=_T[[:digit:]]{1,2}_)Bourgard.+(?=/quant.sf)',
                       grepl(experiments, pattern = 'Meul22') ~ '(?<=_T[[:digit:]]{1,2}_)vvx_.+(?=/quant.sf)',
                       .default  = '(?<=_vvx_T[[:digit:]]{1,2}_).+(?=/quant.sf)'
                     )
                     
                     #Get the assembly names
                     
                     assembly.names <-  unique(str_extract(quant_files, 
                                                    pattern = assembly.pattern))
                     # For each assembly do:
                     lapply(assembly.names, function (assembly.nam){
                       
                       print(paste0("Doing ", assembly.nam))
                       
                       #HMMer file:
                       hmmer.path <- file.path(results.path, 
                                               "hmmer",
                                               paste0(assembly.nam,
                                                      "_hmmer_matches.txt"))
                       
                       #HMMer results - requires a decent amount of processing.
                       hmmer_gz.path <- file.path(results.path, 
                                                  "hmmer",
                                                  paste0(assembly.nam,".txt.gz"))
                       
                       read_HMMertxtgz(hmmer_gz.path)
                       
                       list.files(file.path(results.path, 
                                               "hmmer"))
                       read_table(file.path(results.path, 
                                               "hmmer",
                                               paste0(assembly.nam,".txt.gz")))
                       
                       #Throw error is there is more than one hmmer_path for an assembly, should be impossible.
                       stopifnot(length(hmmer.path) == 1)
                       
                       # Read in the list of HMMer matches, subset to get as a vector.
                       #  Ignore if empty.
                       if(!is_empty(readLines(hmmer.path))){
                         hmmer.list <- read.table(hmmer.path, stringsAsFactors = FALSE)[,1]
                       } else {
                         hmmer.list <- ''
                       }
                       ...
                       fasta.path <- file.path(results.path, 
                                               "evigene", "okayset",
                                               paste0(assembly.nam,
                                                      ".okay.aa"))
                       
                       # Get the quant files for this assembly
                       assembly_quant.files <- str_subset(quant_files,
                       ...
                       lapply(assembly_quant.files, function(qnt){
                         ...
                         read.delim(qnt) %>% 
                           mutate(source = str_extract(qnt, '(?<=/salmon/).+(?=/quant.sf)'),
                                  sample =  sample.nam,
                                  assembly = assembly.nam,
                                  file_source = qnt,
                                  results_path = results.path,
                                  hmmer_path = hmmer.path,
                                  hmmer_match = ifelse(!is_empty(hmmer.list) & Name %in% hmmer.list, 'Yes', 'No'),
...
#. i.e. Filter out every transcript with at leas one sample with >= 1 TPM/NumReads.
...
group_by(quant.df, assembly) %>% 
  summarise(prop = 100*n_distinct(Name[TPM >= 1 & NumReads >= 1])/n_distinct(Name),
            prop_pir = 100*n_distinct(Name[TPM >= 1 & NumReads >= 1 & hmmer_match == 'Yes'])/n_distinct(Name[hmmer_match == 'Yes']),
            num_pir = n_distinct(Name[hmmer_match == 'Yes'])) %>% 
  arrange(prop_pir)
