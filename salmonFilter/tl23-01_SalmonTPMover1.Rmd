...
arrange(tmp,Name)[1,] %>% 
  # filter(Name == 'PVX_001610') %>% 
  mutate(subfam = if_else(any(lopez13_sal1Subfam$Name %in% Name), lopez13_sal1Subfam$subfam[which(lopez13_sal1Subfam$Name %in% Name)], subfam))

tmpid <- tmp$Name

ifelse(tmpid %in% lopez13_sal1Subfam$name, which(lopez13_sal1Subfam$name %in% tmpid), 'poo')

sapply(sort(tmpid), 
       function(tmp_id) {
         if(tmp_id %in% lopez13_sal1Subfam$name) {
           lopez13_sal1Subfam$subfam[which(lopez13_sal1Subfam$name %in% tmp_id)]
         } else {
           'vvxSal1'
         }
       }) %>% cbind %>% data.frame

arrange(lopez13_sal1Subfam, name)
