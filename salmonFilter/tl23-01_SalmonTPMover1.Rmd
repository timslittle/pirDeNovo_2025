...
Heatmap(assigned_subfam_prop.mat[1:26,], 
        na_col = 'white',
        # cluster_rows = FALSE,
        # cluster_columns = FALSE,
        row_names_gp = gpar(fontsize = 6),
        col = circlize::colorRamp2(c(0,0.00001,0.1,1),
                                   c('white', viridis::inferno(3))))
...
assigned_subfam_num.df <- assigned_subfam.df %>% 
  filter(assembly %in% assemblies_overFiftyBUSCO) %>% 
  group_by(expt, assembly, I1.4, new_subfam_uniq) %>% 
  summarise(num_genes = n_distinct(label))

ggplot(assigned_subfam_num.df, aes(x = assembly, y = num_genes, fill = new_subfam_uniq)) +
         geom_col(position = 'fill')
...
assigned_subfam_prop.mat[is.na(assigned_subfam_prop.mat)] <- 0
# max_col <- max(na.omit(assigned_subfam_prop.mat))
Heatmap(assigned_subfam_prop.mat, 
        na_col = 'white',
        # cluster_rows = FALSE,
        # cluster_columns = FALSE,
        row_names_gp = gpar(fontsize = 6),
        ...
        row_split = 4,
        column_split = 6,
        col = circlize::colorRamp2(c(0,0.0001,max(assigned_subfam_prop.mat)),
