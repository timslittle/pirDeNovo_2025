       ...
       function(expt.id){
         ...
         lapply(unique(expt.df$assembly_id), 
                function(assm.id){
                  ...
                  tpm_max_filter <- max(assm.df$TPM)/10
                  assm.df <-  filter(assm.df, TPM_max >= tpm_max_filter)
                  ...
                  tpm.df <- reshape2::dcast(assm.df, label + new_subfam_uniq + TPM_med + id99_ref ~ helpful_sample_name, 
                                            value.var = 'TPM') %>% 
                    ...
                    mutate(new_subfam_uniq = factor(new_subfam_uniq, levels = unique(new_subfam_uniq)),
                           rownames = ifelse(label %in% ancestral_genes_denovo, 'anc', 
                                             ifelse(str_detect(id99_ref, pattern = 'P'), id99_ref, NA ))) %>% 
                    arrange(new_subfam_uniq, desc(TPM_med))
                  
                  tpm.mat <- as.matrix(select(tpm.df, -new_subfam_uniq, -TPM_med, -id99_ref, -rownames, -label))
