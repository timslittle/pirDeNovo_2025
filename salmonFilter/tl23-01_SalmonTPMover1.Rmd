...
filter(blast_filt_dedup.df, 
       ...
       qcovhsp >= 99, 
       (send - sstart)/slen >= .99,
       str_detect(qseqid , pattern = 'Nonam') | str_detect(sseqid , pattern = 'Nonam')) %>% 
  group_by(sseqid) %>% 
  summarise(gene_ident = paste(qseqid, collapse = ', ')) %>% 
  arrange(desc(sseqid) )

group_by(blast_filt_dedup.df, sseqid) %>%  
       known_pir_id = ifelse(pident >= 99 & qcovhsp >= 99 & (send - sstart)/slen >= .99
...
assigned_subfam_tpm.df <- assigned_subfam.df %>% 
  ...
  select(expt, assembly_id, I1.4, new_subfam_uniq, Complete, TPM_med)
...
quant_subfam.df <- inner_join(
  left_join(quant_filtered.df, 
           select(assigned_subfam.df, c('label', 'Name', 'hmmer_path', "TPM_med", "TPM_max", "TPM_min", "NumReads_med", "NumReads_max", 
                                        "NumReads_min", "I1.4", "new_subfam", "num_pir", "num_transcripts", 
                                        "expt", "Input_file", "Dataset", "Complete", "Single", "Duplicated", 
                                        "Fragmented", "Missing", "n_markers", "lifecycle_stage", "location", 
                                        "experiment_id", "new_subfam_uniq", "assembly_id")),
           by = c('Name', 'hmmer_path')),
  sample_helpful_names.df, by = 'sample')
...
lapply(unique(quant_subfam_buscoFifty.df$experiment_id), function(expt.id){
  expt.df <- quant_subfam_buscoFifty.df[quant_subfam_buscoFifty.df$experiment_id == expt.id,]
  lapply(unique(expt.df$assembly_id), function(assm.id){
    assm.df <- expt.df[expt.df$assembly_id == assm.id,] %>% filter(TPM_max >= tpm_max_filter)
    ...
    tpm.df <- reshape2::dcast(assm.df, label + new_subfam_uniq + TPM_med ~ helpful_sample_name, value.var = 'TPM') %>% 
    ...
    tpm.mat <- as.matrix(select(tpm.df, -new_subfam_uniq, -TPM_med))
    subfam.mat <- as.matrix(select(tpm.df, new_subfam_uniq))
    ...
    #Ancestral gene name only
    rownames(tpm.mat) <- ifelse(rownames(tpm.mat) %in% ancestral_genes_denovo,
                                'anc',
                                '')
    
    draw( Heatmap(tpm.mat, 
                  cluster_rows = FALSE, 
                  cluster_columns = FALSE,
                  ...
                  col = circlize::colorRamp2(c(0,max(tpm.mat)/2,max(tpm.mat)),
                                   c('white',viridis::viridis(2)))) 
          # + Heatmap(subfam.mat, 
...
TODO:
* ~Get ordering of heatmaps right.~
* There's something wrong with the colour scale of the heatmaps, can be seen when changing filter.
* Add cell cycle markers/gametocyte/ookinete markers to the expression. - Use BUSCO results from full_table.tsv.
