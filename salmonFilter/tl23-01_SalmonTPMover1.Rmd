...
experiment.vec <- c('Bourgard21', 'Imp22', 'K19', 'Meul22', 'R20', 'S20-Z16', 'B20-C19-G19-G18-M19-K17')
         ...
         lapply(quant_files, function(qnt){
           ...
           read.delim(qnt) %>% 
             mutate(source = str_extract(qnt, '(?<=/salmon/).+(?=/quant.sf)'),
                    sample = ifelse(grepl(qnt, pattern = 'Bourgard'),
                                    str_extract(qnt, '(?<=/salmon/).+(?=(_Bourgard).+/quant.sf)'),
                                    str_extract(qnt, '(?<=/salmon/).+_vvx_T[[:digit:]]+(?=.+_vvx/quant.sf)')),
                    assembly = ifelse(grepl(qnt, pattern = 'Bourgard'),
                                    str_extract(qnt, '(?<=/salmon/(Bourgard))(Bourgard).+(?=/quant.sf)'),
                                    str_extract(qnt, '(?<=_vvx_T[[:digit:]]{1,2}_).+(?=/quant.sf)')),
                    file_source = qnt) 
         }) %>% bind_rows
       }) %>% bind_rows
