...
quant.df <- read_csv(file = 'salmonQuant_all.csv')
...
quant_filtered.df <- group_by(quant.df, 
  ...
  filter(!is.na(hmm_cvrg) & hmm_cvrg >= 75)

hist(quant_filtered.df$hmm_cvrg_perc, breaks = 1000)
...
group_by(quant.df, assembly) %>% 
  summarise(prop = 100*n_distinct(Name[TPM >= 1 & NumReads >= 1])/n_distinct(Name),
            ...
            prop_pir_hmm = 100*n_distinct(Name[hmm_cvrg >= 75 & !is.na(Sequence)])/n_distinct(Name[!is.na(Sequence)]),
            prop_pir_filt = 100*n_distinct(Name[TPM >= 1 & NumReads >= 1 & !is.na(Sequence) & hmm_cvrg >= 75])/n_distinct(Name[!is.na(Sequence)]),
            num_pir = n_distinct(Name[!is.na(Sequence)])) %>% 
  arrange(prop_pir_filt)

experiments <- 'B20-C19-G19-G18-M19-K17'
assembly.nam <- 'Gunalan19_Sal1_vvx'
