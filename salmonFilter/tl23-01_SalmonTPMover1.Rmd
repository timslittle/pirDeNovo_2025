...
```{r assembly_stats}
assembly_numTranscript <- quant_filtered_sequence.df %>% 
  group_by(assembly,fasta_path) %>% 
  summarise(num_pir = n_distinct(Name)) %>% 
  arrange(desc(num_pir)) %>% 
  mutate(num_transcripts = system(paste0('grep ">" ',fasta_path,'| wc -l'), intern = TRUE),
         expt = str_extract(assembly, pattern = '^.+(?=_.+_vvx)'))

assembly_busco <- lapply(unique(quant_filtered_sequence.df$hmmer_path), 
       function(hmmer.path){
         res.path = str_extract(hmmer.path, pattern = '.+vvx_.+(?=/hmmer)')
         assembly.nam = unique(quant_filtered_sequence.df$assembly[quant_filtered_sequence.df$hmmer_path == hmmer.path])
...
ggplot(data = assembly_stats.df, aes(x = Complete, 
                                     y = num_pir, 
                                     col = expt)) +
  geom_point(size = 4) +
  theme_classic()
