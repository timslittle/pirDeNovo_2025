...
```{r lm}
colnames(assigned_subfam.df)
assigned_subfam.df[is.na(assigned_subfam.df$experiment_id),]

#Add the missing values (i.e. the zero TPM values for missing sub-families)
subfam_prop_zero.df <- select(assigned_subfam.df, 
       label,
       assembly_id, 
       new_subfam_uniq,
       Complete,
  ...
  tidyr::complete(assembly_id, new_subfam_uniq, fill = list(TPM_med = 0)) %>% 
  group_by(assembly_id) %>% 
  arrange(Complete) %>% 
  # Fill in the NA values for these constant values across assemblies.
  tidyr::fill(Complete, experiment_id, country, lifecycle_stage)

# filter(subfam_prop_zero.df, is.na(Complete))

lm.df <- group_by(subfam_prop_zero.df, 
                  assembly_id, 
                  new_subfam_uniq,
                  Complete,
                  experiment_id,
...
leaps.m <- leaps::regsubsets(prop_tpm ~ ., lm.df, method = "forward")
summary(leaps.m)

prop_tpm.lm <- lm(prop_tpm ~ 0 + new_subfam_uniq*country*lifecycle_stage + Complete + experiment_id, 
                  data = lm.df)
summary(prop_tpm.lm)
prop_tpm.coef <- coef(summary(prop_tpm.lm)) 
prop_tpm.coef[prop_tpm.coef[,"Pr(>|t|)"] < .05,]
arrange(as.data.frame(prop_tpm.coef[prop_tpm.coef[,"Pr(>|t|)"] < .05,], desc('Pr(>|t|)')))
```
...
** If including BUSCO and stages in the linear model I need all the zero values too.
