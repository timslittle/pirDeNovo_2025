...
subfam.df <- data.frame(Name = deNovo_Ref_pirs.list,
                        subfam = case_when(
                          grepl(deNovo_Ref_pirs.list, pattern = 'Nonam') ~ "de_novo",
                          ...
                          grepl(deNovo_Ref_pirs.list, pattern = 'MKS88') ~ "bras"),
                        stringsAsFactors = FALSE) %>%
  mutate(subfam = ifelse(subfam == 'vvxSal1' & Name %in% lopez13_sal1Subfam$Name, 
                         as.character(lopez13_sal1Subfam$subfam[lopez13_sal1Subfam$Name %in% Name]),
                         subfam))
```
