...
quant.df <- lapply(experiment.vec,
                   function(experiments){
                     ...
                     quant_files <- list.files(path = results.path,
                                               recursive = TRUE,
                                               pattern = 'quant.sf',
                                               full.names = TRUE) %>% 
...
# How many genes are kept for each assembly
group_by(quant.df, assembly) %>% 
  summarise(prop = 100*n_distinct(Name[TPM >= 1 & NumReads >= 1])/n_distinct(Name) ) %>% 
  arrange(prop)

unique(quant.df$hmmer_path)

```

Combine the expression data with the HMMer pirs.

```{r hmmer_results}

lapply(unique(quant_filtered.df$assembly)[1], 
       function(assembly.name){
         hmmer_path <- unique(quant_filtered.df$hmmer_path[quant_filtered.df$assembly == assembly.name])
         
         #Throw error is there is more than one hmmer_path for an assembly, should be impossible.
         stopifnot(length(hmmer_path) == 1)
         
         # Read in the list of HMMer matches, subset to get as a vector.
         hmmer.list <- read.table(hmmer_path, stringsAsFactors = FALSE)[,1]
