...
quant.df <- lapply(experiment.vec,
       function(experiments){
         ...
         #Some of the files are from erroneous runs of the software, can grep for the 'T1' etc name to find those ran with the more modern pipeline. TODO: Remove the old files.
         ...
         lapply(quant_files, function(qnt){
           #Read in the data and get columns for the experimental data
           read.delim(qnt) %>% 
             mutate(source = str_extract(qnt, '(?<=/salmon/).+(?=/quant.sf)'),
                    sample =  str_extract(qnt,
                                          sample.pattern),
                    assembly = str_extract(qnt,
                                           assembly.pattern),
                    file_source = qnt,
...
```{r hmmer_results}
list_files.path <- Sys.glob("../../nextflow/vvx_*")

lapply(unique(quant_filtered.df$assembly), 
       function(assembly.name){
         list.files(path = "", recursive = TRUE, pattern = assembly.name)
  read.table()
})
```
