...
```{r load_salmon_quant_files}
#Experimental assembly names (the old ones that I was using during analysis)
# experiment.vec <- c('Bourgard21')
experiment.vec <- c('Bourgard21', 'Imp22', 'K19', 'Meul22',
                    'R20', 'S20-Z16', 'B20-C19-G19-G18-M19-K17')

#USER: Replace the path names after 'ls' with the pathnames to the salmon quant file outputs.
quant.files <- c(
...
#Get the list of the different assembly files 
# (the folder names for the quant.sf files, as each alignment was performed against these assemblies.)
...
# 
#                     #If the 'busco_assembly_name' is present, import that. Otherwise use assembly.nam.
...
sample_helpful_names.df <- read.csv('sample_helpfulNames.csv') %>% 
  select(id, Experiment, helpful_sample_name, samplename, )

quant_small.df <- select(quant.df, Name, Length.x, TPM, NumReads, sample, E.value_fullseq, Description.y, expt, 
                         location, experiment_id, 
                         helpful_assembly_name, helpful_id, country)

quant_helpful.df <- merge(sample_helpful_names.df, quant_small.df) %>% 
  select(sample, id, Experiment, helpful_sample_name, Name, 
         Length.x, TPM, NumReads, E.value_fullseq, Description.y, expt, 
         location, experiment_id, 
         helpful_assembly_name, helpful_id, country)

quant_helpful.df %>% mutate(sample_name = paste0(helpful_id, '_', helpful_sample_name)) %>% tail()
...
```{r quant_filtered}
## How the quant data frame was filtered (TPM >=1 in alignment/counting to any sample,
## and HMMer coverage >= 75% OR in the comp_busco_genes list)
# group_by(quant.df,
#                               assembly,
#                               fasta_path,
#                               Name) %>%
...
### Prev issue: "Somehow only get the median TPMs once (just before the heatmaps) instead of here then getting median again across the sub-family."
## I don't think I actually do this, only calculate TPM_med the once. I do calc the prop_TPM using the sum of TPMs, so hopefully that is okay.
## May be worth repeating the heatmaps with all samples instead of the medians but that is not a priority.
...
subfam_graph.df <- gephi_mcl.df %>% 
  ...
  #                      subfam, 
  #                      'none')) %>% 
  group_by(subfam, species, infl_net, .drop = FALSE) %>% 
  dplyr::summarise(number = n_distinct(label))
...
# assigned_subfam_uniqName.df$new_subfam_uniq <- paste0(assigned_subfam_uniqName.df$I1.4,ifelse(assigned_subfam_uniqName.df$new_subfam == 'new',
#                                                                                               '', paste0('_', assigned_subfam_uniqName.df$new_subfam) ) )
...
```{r}
# Get the new sub-family names in the quant_filtered.df so that we have a data frame of each pir gene, it's expression, and its sub-family.
#   Note that this has fewer rows than 'assigned_subfam.df' because it will only have the de_novo assembled pirs and not other 
#     species/reference genes.

quant_filtered_withNewSubfam.df <- mutate(quant_filtered.df,
                                          Name_assembly = paste0(Name, '_', assembly)) %>% 
  inner_join(.,
             select(assigned_subfam.df,
                    -c(starts_with('TPM')|starts_with('NumReads'))), 
             by = colnames(assigned_subfam.df)[colnames(assigned_subfam.df) %in% colnames(.)])

#Sanity check
length(unique(quant_filtered_withNewSubfam.df$Name_assembly)) == 6513

# write_csv(quant_filtered_withNewSubfam.df, 'pirLabel_newSubfam_quant.csv')
```
...
pdf("../../plots/SuppFig_vvx_Subfam-Propheatmap_orderCluster.pdf", width = 7, height = 5)
