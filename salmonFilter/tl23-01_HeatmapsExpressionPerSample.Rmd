...
quant_subfam.df <- filter(quant_subfam.df, 
                          grepl(Description.y, pattern = regex('oocyst|ookinete|gamete', ignore_case = TRUE)) | is.na(Description.y) ) %>% 
       ...
       function(expt.id){
         ...
         lapply(unique(expt.df$assembly_id), 
                function(assm.id){
                  ...
                  if(is.null(dim(tpm_ref.mat))) {
                    # Only 1D vector.
                    tpm_ref.mat <- tpm_ref.mat[tpm_ref.mat > tpm_ref.filter]
                  }
                  else if(length(tpm_ref.mat) >= 1){
                    tpm_ref.mat <- tpm_ref.mat[apply(tpm_ref.mat,1,sum) > tpm_ref.filter,]
                  }
