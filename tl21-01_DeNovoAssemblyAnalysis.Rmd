...
There are `r length(unique(transcripts_covered_by_denovo$geneid))` known transcripts covered by >=95% of at least one individual _de novo_ transcript. There is a mean of `r mean(transcripts_covered_by_denovo$mismatch)` mismatches and `r mean(transcripts_covered_by_denovo$gapopen)` gaps and a median of `r median(transcripts_covered_by_denovo$mismatch)` and `r median(transcripts_covered_by_denovo$gapopen)` in these alignments (Note that there are probably some de novo transcripts that align >=95% to the same known transcript). Of these `r length(unique(transcripts_covered_by_denovo$geneid[transcripts_covered_by_denovo$num_match_all == 1]))`
 only match that known transcript and no others by the BLAST search. 
